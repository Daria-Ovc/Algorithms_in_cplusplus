// Нелинейные системы. 
// Внешние итерации по Зейделю, внутренние по Ньютону.
...
Решаем СНАУ:
 ...
 ...
 fn(x1..xn) = 0
 размерности n*n (в программе размерность 2*2).
 В виде операторного уравнения: F(x) = 0.
*/
...
const int n = 2;    //размерность СНАУ
...
void Inverse_Matrix(double** A) 
{
    ...
    tmp = A[0][0];
    if (abs(det) > EPSILON)  //проверка det<>0
    {
        A[0][0] = A[1][1] / det;
        A[1][1] = tmp / det;
        A[0][1] = -A[0][1] / det;
...
//метод Зейдель-Ньютон
void Seidel_Newton(double x, double y, double& x_new, double& y_new) 
{
    double D11, D22;  //диагональные элементы матрицы Якоби
...
int main()
{
    ...
    double initial[2];   //начальное приближение
    initial[0] = -1.2;
    initial[1] = -1.8;
    double new_answ[2];  //полученное решение при очередной итерации
    double max, norma;   //разницы между старым и новым решением
    ...
    do
    {
        ...
        //Находим максимальную разницу между переменными
        ...
        i++;
        cout << "Итерация № " << i << endl << new_answ[0] << "\t" << new_answ[1] << endl << endl;

    } while ((max > EPSILON || norma > EPSILON) && i < 1000);

    cout << endl << endl;
    Answer(new_answ[0], new_answ[1]);  //вывод ответа
